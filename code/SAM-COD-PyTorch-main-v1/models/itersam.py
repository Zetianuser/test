...
@register('sam_with_iter')
class IterSAM(nn.Module):
    ...
    def forward(self):
        ...
        self.pred_mask_init = masks
        mask_prompt = torch.sigmoid(low_res_masks)
        #points = self.get_points_from_pred(self.pred_mask_init)
    ...
    def infer(self, input):
        ...
        # Upscale the masks to the original image resolution
        # masks双线性插值至输入图像大小，根据原图像大小裁剪masks，去掉padding区域
        pred_mask_init = self.postprocess_masks(low_res_masks, self.inp_size, self.inp_size)

        mask_prompt = torch.sigmoid(low_res_masks)
        #points = self.get_points_from_pred(pred_mask_init)
        ...
        # Upscale the masks to the original image resolution
        # masks双线性插值至输入图像大小，根据原图像大小裁剪masks，去掉padding区域
        pred_mask = self.postprocess_masks(low_res_masks, self.inp_size, self.inp_size)
    ...
    def backward_G(self):
        ...
        self.loss_G = 0.5 * self.criterionBCE(self.pred_mask_init, self.gt_mask)
        self.loss_G += self.criterionBCE(self.pred_mask, self.gt_mask)
        if self.loss_mode == 'iou':
            self.loss_G += 0.5 * _iou_loss(self.pred_mask_init, self.gt_mask)
    ...
    def get_points_from_pred(self, pred):
        binary_mask = pred > self.mask_threshold
        batch_size, _, H, W = pred.shape
        mask_array = np.uint8(binary_mask.data.cpu().numpy())
        batch_centers = []
        batch_labels = []
        for i in range(batch_size):
            gt = mask_array[i, 0, :, :]
