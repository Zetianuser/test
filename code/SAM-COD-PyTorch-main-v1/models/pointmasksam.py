...
class AutomaticSAM(nn.Module):
    ...
    def forward(self):
        ...
        points = self.get_points_from_gts(self.gt_mask)
        mask_prompt = F.interpolate(
            self.gt_mask,
            (4 * self.image_embedding_size, 4 * self.image_embedding_size),
            mode='bilinear',
            align_corners=False,
        )
    ...
    def infer(self, input, gt):
        ...
        points = self.get_points_from_gts(gt)
        mask_prompt = F.interpolate(
            gt,
            (4 * self.image_embedding_size, 4 * self.image_embedding_size),
            mode='bilinear',
