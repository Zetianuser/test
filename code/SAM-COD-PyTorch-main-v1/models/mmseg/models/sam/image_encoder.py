...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # 图像嵌入
        # (N, C, H*W)->(N, H*W, C//scale_factor)
        embedding_feature = self.prompt_generator.init_embeddings(x)
        # 图像傅里叶变换后embedding，额外的频域prompt
        # (N, C//scale_factor, H. W)
...
class PromptGenerator(nn.Module):
    ...
    def get_prompt(self, handcrafted_feature, embedding_feature):
        ...
        for i in range(self.depth):
            # getattr获取属性值
            # 获取lightweight_mlp_i的属性值到self
            # lightweight_mlp为Linear、GELU
...
class PatchEmbed2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        # 转为元组 image_size=1024 patch_size=16 num_patches=64*64 embed_dim=768//32=24
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        #self.attention_prompt1 = nn.Sequential(
        #    nn.Conv2d(self.num_heads, dim, 1),
        #    nn.GELU(),
        #)
        #self.attention_prompt2 = nn.Sequential(
        #    nn.Conv2d(dim, self.num_heads, 1),
        #    nn.GELU(),
        #)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.window_size > 0:
            ...
            attn = attn_reverse_unpartition(attn, self.window_size, pad_hw, (H, W), self.num_heads)
        else:
            attn = attn_reverse(attn, x.shape[1], self.num_heads)

        #attn_prompt = self.attention_prompt1(attn)
        #attn = self.attention_prompt2(attn_prompt)
        #x = shortcut + x + attn_prompt.detach().permute(0, 2, 3, 1).contiguous()
        x = shortcut + x
...
def attn_reverse_unpartition(
    attn: torch.Tensor, window_size: int, pad_hw: Tuple[int, int], hw: Tuple[int, int], heads: int
) -> torch.Tensor:

    Hp, Wp = pad_hw
    H, W = hw
    attn_dim = window_size * window_size
    N = attn.shape[0]
    assert (attn.shape[1] == attn_dim and attn.shape[2] == attn_dim),\
        'The shape of attn must match with the window size.'
    #attn = attn.mean(axis=1).squeeze(1).reshape(-1, window_size, window_size)
    x = attn.mean(axis=1).reshape(N, window_size, window_size)
    #attn = attn.squeeze(1)
    #attn = attn.reshape(-1, window_size, window_size)
    num_window_h = Hp // window_size
    num_window_w = Wp // window_size
    BH = N // (num_window_h * num_window_w)
    B = BH//heads
    x = x.view(N//heads, heads, window_size, window_size).permute(0, 2, 3, 1).contiguous()
    x = x.view(B, num_window_h, num_window_w, window_size, window_size, -1)
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, Hp, Wp, -1)
    x = x.permute(0, 3, 1, 2).contiguous().view(B, heads, Hp, Wp)
...
def attn_reverse(
        attn: torch.Tensor, num_patch: int, heads: int
) -> torch.Tensor:
    BH, size_square, _ = attn.shape
    B = BH // heads

    assert (attn.shape[1] == num_patch * num_patch), \
        'The shape of attn must match with the patch size.'

    x = attn.mean(axis=1).reshape(BH, num_patch, num_patch).view(B, heads, num_patch, num_patch)
