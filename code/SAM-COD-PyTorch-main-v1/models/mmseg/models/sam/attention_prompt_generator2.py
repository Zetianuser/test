...
class AttentionPromptGenerator2(nn.Module):
    """
    ...
    def __init__(
        ...
        self.final_block = Block2(in_dim=self.cat_dim2, out_dim=64, num_heads=self.num_global_attn, sample_ratio=sample_ratio)
        self.pred = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.pred1 = nn.Conv2d(self.num_global_attn * self.num_heads, 1, 1)
        self.pred2 = nn.Conv2d(self.depth * self.num_heads, 1, 1)
        self.pred3 = nn.Conv2d(self.num_cat_blocks * self.num_heads, 1, 1)
    ...
    def forward(self, attn_list):
        if self.mode == '0':
            ...
            attns = [attn_list[index] for index in self.global_attn_indexes]
            cat_attns = torch.cat(attns, dim=1)
            map = self.pred1(cat_attns)
        elif self.mode == '2':
            cat_attns = torch.cat(attn_list, dim=1)
            map = self.pred2(cat_attns)
        elif self.mode == '3.0':
            i = 0
            cat_attns = torch.cat(attn_list[i*self.num_cat_blocks:(i+1)*self.num_cat_blocks], dim=1)
            map = self.pred3(cat_attns)
        elif self.mode == '3.1':
            i = 1
            cat_attns = torch.cat(attn_list[i*self.num_cat_blocks:(i+1)*self.num_cat_blocks], dim=1)
            map = self.pred3(cat_attns)
