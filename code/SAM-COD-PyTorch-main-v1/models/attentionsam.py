...
@register('sam_with_attention_prompt')
class AttentionSAM(nn.Module):
    ...
    def forward(self):
        ...
        final_attn_map = torch.as_tensor(attention_maps[self.global_attn_indexes[3]], device=self.input.device)
        final_attn_map = F.interpolate(final_attn_map, scale_factor=4, mode='bilinear', align_corners=False)
        attn_prompt = torch.sigmoid(self.attention_map(final_attn_map))
    ...
    def infer(self, input):
        ...
        cache = get_local.cache
        attention_maps = cache['Block.forward.attn']
        final_attn_map = torch.as_tensor(attention_maps[self.global_attn_indexes[3]], device=input.device)
        final_attn_map = F.interpolate(final_attn_map, scale_factor=4, mode='bilinear', align_corners=False)
        attn_prompt = torch.sigmoid(self.attention_map(final_attn_map))
