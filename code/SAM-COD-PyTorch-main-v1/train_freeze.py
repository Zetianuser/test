...
def make_data_loader(spec, tag=''):
    ...
    loader = DataLoader(dataset, batch_size=spec['batch_size'],
                        shuffle=False, num_workers=48, pin_memory=True, sampler=sampler)
    return loader
...
def eval_psnr(loader, model, eval_type=None):
    ...
    with torch.no_grad():
        ...
        result1, result2, result3, result4 = sum(gather_results1) / len(loader.dataset), sum(gather_results2) / len(
            loader.dataset), sum(gather_results3) / len(loader.dataset), sum(gather_results4) / len(loader.dataset)
...
def main(config_, save_path, args):
    ...
    if local_rank == 0:
        ...
        log('model_total_params:{}'.format(model_total_params))
...
if __name__ == '__main__':
    ...
    # 模型设置及超参
    parser.add_argument('--config', default="configs/cod-sam-vit-h-attention-prompt_v1.yaml")
